...
export class EmpService {
  ...
  stmt(id){
    ...
    return this.http.get("http://localhost:8086/account-ms/getAccountStatement/"+id,{headers:header}); 
  }
  range(id,start,end){
    let header = new HttpHeaders().set(
      "Authorization",localStorage.getItem("token")
    ...
    return this.http.get("http://localhost:8086/account-ms/getAccountStatement/"+id+"/"+start+"/"+end,{headers:header}); 
  }
